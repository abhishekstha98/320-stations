...
class WeatherDatasetLoader(object):
    ...
    def _get_targets_and_features(self):
        ...
        self.targets = [
            np.expand_dims(stacked_target[i:(i + self.lags), : ,[0, 1, 2, 4, 5, 7, 8]].T, axis=0)
            for i in range(stacked_target.shape[0] - self.lags)
        ]
        ...
        self.targets = [
            ...
            np.expand_dims
                (np.transpose(stacked_target[i + self.lags:(i + self.lags + self._pred_seq), : ,[0, 1, 2, 4, 5, 7, 8]].T, (2, 1, 0)),
                axis=0)
            for i in range(stacked_target.shape[0] - self.lags- self._pred_seq)
        ]
        '''

    def get_dataset(self, lags: int = 60) -> StaticGraphTemporalSignal:
        self.lags = lags
...
class STGCN(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.stconv_block2 = STConv(750, 128, 256, 512, 7,
                                    4)  # Second last is Kernel sizeself.fc = torch.nn.Linear(256, 3)
...
class STGCN_Best_Babu(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.stconv_block2 = STConv(750, 64, 128, 256, 7,
                                    3)  # Second last is Kernel sizeself.fc = torch.nn.Linear(256, 3)
...
if __name__ == '__main__':
    ...
    #dataset = loader.get_dataset(lags=46, pred_seq=10)  # 'pred_seq' is the number of output seq.
    ...
    Output_Save_path = "E:/WEATHER_FORECAST/Outcome/STGCN_60Lags_10Out_750Stations_Adam/"
    ...
    # model = STGCN_OneMOneW().to(device)
    model = STGCN_Best_Babu().to(device)
    print(model)
    '''
    #Load pre-trained model
    weight_path='E:/ICIMOD_Project/ICIMOD PROJECT/YASH_PROCESS_320_LAST/STGCN_43Lags_7Out_320Stations_Adam/Model_Best_PreTrained.pt'
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        with torch.no_grad():
            ...
            if total_val_loss < best_MSE:
                ...
                torch.save(model.state_dict(), Output_Save_path + 'Model_60Lags_STConv_Best.pt')
    ...
    # Save model weight
    torch.save(model.state_dict(), Output_Save_path + "Model_60Lags_STConv_Last.pth")
